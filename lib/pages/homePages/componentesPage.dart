...
class ComponentesPage extends StatefulWidget {
  ...
  const ComponentesPage({Key? key, required this.accessToken, required this.userEmail}) : super(key: key);
...
class _ComponentesPageState extends State<ComponentesPage> {
  ...
  void initState() {
    super.initState();
    _loadProducts();
    print('Token na página ComponentesPage: "${widget.accessToken}"');
  }

  Future<void> _loadProducts() async {
    try {
      _products = ProductApiService.getProducts(widget.accessToken, 2);
      await _products;
    } catch (error) {
                      ...
                      final result = await showDialog(
                        ...
                        builder: (BuildContext context) {
                          return AddProductModal(
                            accessToken: widget.accessToken,
                            categoryId: 2, // Aqui o categoryId é 2 para componentes
                          );
                        },
                      );
            ...
            children: products.map<Widget>((product) {
              ...
              return CrudContainer(
                ...
                onDelete: () {
                  setState(() {
                    _products =
                        ProductApiService.getProducts(widget.accessToken, 2);
                  });
                },
              );
            }).toList(),
