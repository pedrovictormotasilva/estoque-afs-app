...
class CabosPage extends StatefulWidget {
  ...
  const CabosPage({Key? key, required this.accessToken, required this.userEmail})
      : super(key: key);
...
class _CabosPageState extends State<CabosPage> {
  ...
  void initState() {
    super.initState();
    _loadProducts();
    print('Token na página CabosPage: "${widget.accessToken}"');
  }

  Future<void> _loadProducts() async {
    try {
      _products = ProductApiService.getProducts(widget.accessToken, 5); // Aqui o categoryId é 5
      await _products;
    } catch (error) {
                      ...
                      final result = await showDialog(
                        ...
                        builder: (BuildContext context) {
                          return AddProductModal(
                            accessToken: widget.accessToken,
                            categoryId: 5, // Aqui o categoryId é 5
                          );
                        },
                      );
